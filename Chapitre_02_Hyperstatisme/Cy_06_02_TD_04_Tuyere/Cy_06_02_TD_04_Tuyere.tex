% "{'classe':('PSI'),'chapitre':'chs_hs','type':('td'),'titre':'Tuyère à ouverture variable', 'source':'Banque PT - SIA 2011','comp':('C1-02','C2-04'),'corrige':False}"
...
\begin{marginfigure}
\includegraphics[width=\linewidth]{fig_04_bis}
%\textit{}
\end{marginfigure}


\question{Compléter le graphe de structure.}% en donnant chacune des liaisons et précisant les torseurs cinématiques et statiques. }
\ifprof
\begin{corrige}~\\
\begin{center}
...
\question{Le modèle proposé est-il isostatique ?}

\question{Déterminer la liaison équivalente $\text{L}_{\text{eq}34}$ aux deux liaisons $\text{L}_{3}$ et $\text{L}_{4}$ situées entre le solide 1 et le solide 3. On attend une démonstration par le calcul. On précisera la forme du torseur des actions transmissibles $\mathcal{F}_{\text{eq}34}$.}
\ifprof
\begin{corrige}~\\
...
\question{Déterminer la liaison équivalente $\text{L}_{\text{eq}12}$ aux deux liaisons $\text{L}_{1}$ et $\text{L}_{2}$ situées entre le solide 1 et le solide 3. On attend une démonstration par le calcul. On précisera la forme du torseur des actions transmissibles $\mathcal{F}_{\text{eq}12}$.}
\ifprof
\begin{corrige}~\\
\end{corrige}
\else
...
\question{Déterminer la liaison équivalente $\text{L}_{\text{eq}12}$ aux deux liaisons $\text{L}_{\text{eq}34}$ et $\text{L}_{\text{eq}12}$ situées entre le solide 1 et le solide 3. On attend une démonstration par le calcul. On précisera la forme du torseur des actions transmissibles $\mathcal{F}_{\text{eq}}$.
...
\question{À partir du graphe de structure (graphe des liaisons) et en vous inspirant des résultats trouvés précédemment déterminer la liaison équivalente $\text{L}_{\text{eq}1}$
...
\question{Déterminer par la méthode de votre choix, la liaison équivalente $\text{L'}_{\text{eq}1}$ aux deux liaisons $\text{L}_{\text{eq}34}$, $\text{L}_{\text{eq}1}$ et $\text{L}_{\text{eq}2}$ situées
...
\begin{marginfigure}
\centering
\includegraphics[width=3cm]{Cy_06_02_TD_04_Tuyere_qr}
\end{marginfigure}
\fi



\question{Pour des raisons d'encombrement des vérins et de capacité à fournir les actions mécaniques de poussée, le
